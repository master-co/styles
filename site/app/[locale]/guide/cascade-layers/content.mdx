...
- `css @layer base, theme, preset, styles, general;` declares the order of the style layers. The cascade priority follows this order, where layers declared later override earlier ones if there are conflicting styles.
...
- `css @layer styles { … }` generates rules based on used abstract styles (`mcss btn`) and `config.styles`.
...
(x) Don't add brand's preset styles to the base layer.
```html
...
You can override the component [styles](/guide/styles) by adding general syntax, without having to prioritize rules.
