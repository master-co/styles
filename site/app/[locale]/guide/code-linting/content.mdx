...
You can also restrict teams to using [configuration](/reference/configuration) to create [styles](/guide/custom-styles) instead of defining classes through traditional `html <style>` or CSS files.
...
```
(o) Or create the custom style `btn` using the [styles](/guide/custom-styles) configuration.
