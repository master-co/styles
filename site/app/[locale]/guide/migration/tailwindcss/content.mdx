...
```
Not just media queries but feature queries and [more](/reference/custom-syntax).

---

## Configuration
### Add screens
Define responsive breakpoints using variable [screens](/reference/using-variables#add-a-screen-size).
...
### Add media queries
Using [at-rules](/reference/custom-syntax#at-rules) to define media queries, feature queries, and even container queries.
```js name=master.css.js
-/** @type {import('tailwindcss').Config} */
-module.exports = {
-    theme: {
...
Use [styles](/guide/custom-styles) to abstract styles instead of `@apply`.
```css name=main.css
- @tailwind base;
- @tailwind components;
- @tailwind utilities;
