...
Explicitly specify sources to scan, ignoring [`options.include`](/reference/extractor#include) and [`options.exclude`](/reference/extractor#exclude) restrictions.
...
The source path of the Master CSS [configuration](/reference/configuration).
<DocProp defaultValue="'master.css.*'" types={['string']} />
```bash name=Terminal
