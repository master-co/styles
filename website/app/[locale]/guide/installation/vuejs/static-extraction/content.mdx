import StaticExtractionIntro from '../../../rendering-modes/components/StaticExtractionIntro.mdx'

<StaticExtractionIntro img={{
    href: "https://vuejs.se.rc.css.master.co",
    ...require('~/website/public/images/vuejs-dev.png').default,
    dark: require('~/website/public/images/vuejs-dev@dark.png').default,
    alt: 'Vue.js and Master CSS',
    width: 665, height: 400 }} />

---

## Quick start
<StepSection>
    <Step $row>
        <StepL>
            ### <StepNum />Clone the example
            Copy-paste the commands to quickly start using the https://vuejs.se.rc.css.master.co example.

            You can skip all [installation](#installation) steps.
        </StepL>
        <StepR>
            ```bash name=Terminal
            npm create @master/css@rc __project__ --example vue.js-with-static-extraction
            cd __project__
            npm run dev
            ```
        </StepR>
        <StepEnd/>
    </Step>
</StepSection>

---

## Installation

<StepSection>
    <Step $row>
        <StepL>
            ### <StepNum />Create a project
            If you don't have a Vue.js project, create one first. It's recommended to refer to [Quick Start - Vue.js](https://vuejs.org/guide/quick-start.html).
        </StepL>
        <StepR>
            ```bash name=Terminal
            npm init vue@latest
            cd __project__
            ```
        </StepR>
    </Step>
    <Step $row>
        <StepL>
            ### <StepNum />Initialize configuration file
            Run `npm create @master/css@rc` to create a configuration file [master.css.ts](/reference/configuration).
        </StepL>
        <StepR>
            ```bash name=Terminal
            npm create @master/css@rc
            ```
        </StepR>
    </Step>
    <Step $row>
        <StepL>
            ### <StepNum />Install Master CSS
            Install the Master CSS extractor into your project via package managers.
        </StepL>
        <StepR>
            ```install
            **@master/css-extractor.vite@rc**
            ```
        </StepR>
    </Step>
    <Step $row>
        <StepL>
            ### <StepNum />Set up CSS extractor
            - Create a `vite.config.ts` file
            - Add a `CSSExtractorPlugin` to the vite `plugins`
        </StepL>
        <StepR>
            ```ts name=vite.config.ts
            +import { defineConfig } from 'vite'
            +import CSSExtractorPlugin from '@master/css-extractor.vite'

            +export default defineConfig({
            +    plugins: [
            +        CSSExtractorPlugin()
            +    ]
            +})
            ```
        </StepR>
    </Step>
    <Step $row>
        <StepL>
            ### <StepNum />Import virtual CSS module
            Import the virtual CSS module `.virtual/master.css` into the `main.ts`.
        </StepL>
        <StepR>
            ```ts name=main.ts
            …
            +import '.virtual/master.css';
            ```
        </StepR>
    </Step>
    <Step $row>
        <StepL>
            ### <StepNum />Launch server
            Run `npm run dev` to start your Vite development server
        </StepL>
        <StepR>
            ```bash name=Terminal
            npm run dev
            ```
        </StepR>
    </Step>
    <Step>
        ### <StepNum />Start using Master CSS
        Now style your first element using Master CSS syntax!
        ```html name=src/App.vue
        …
        <template>
            <CSSRuntimeProvider :config="config">
                <h1 class="**font:40** **font:heavy** **italic** **m:12x** **text:center**">Hello World</h1>
            </CSSRuntimeProvider>
        </template>
        ```
        <p className="mt:12x">Open your browser to watch the changes.</p>
        <HelloWorld url="localhost:3000" />
    </Step>
</StepSection>