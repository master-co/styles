## Foreword [sr-only]
The [Language Server](/reference/language-server) and [ESLint](/reference/eslint) both support the monorepo development strategy.
...
When there are different design systems or complex requirements in a monorepo, you may need to create __master.css.js__ and __eslint.config.js__ for multiple folders to serve as [workspaces](/reference/language-server#settingsworkspaces).
