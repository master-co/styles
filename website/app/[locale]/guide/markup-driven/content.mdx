...
Design token is a core methodology for practicing design systems, so appropriately abstracting styles and markup can help you create visually consistent user interfaces more effectively, and Master CSS [configuration](/reference/configuration) is born for this.

Assuming the card can be reused, you can abstract it using [styles](/reference/styles) with a concise syntax:
...
<div class="block">
```
It can be [stateful and selectable](/reference/selectors):
```html
...
```
It can be [responsive](/guide/responsive-design) and [conditional](/reference/queries):
```html
