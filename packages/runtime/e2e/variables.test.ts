...
test('expects the variable output', async ({ page }) => {
    ...
    ).toContain(':root{--first:17 17 17}.dark{--first:34 34 34}.light, :root{--first:51 51 51}.bg\\:first{background-color:rgb(var(--first))}')
    ...
    expect(text).toMatch(/\.light, :root\{[^}]*--third:119 119 119[^}]*\}/)
    ...
    expect(text).toMatch(/\.light, :root\{[^}]*--fourth:0 0 0[^}]*\}/)
    ...
    expect(text).not.toMatch(/\.light, :root\{[^}]*--third:119 119 119[^}]*\}/)
    ...
    expect(text).not.toMatch(/\.light, :root\{[^}]*--fourth:0 0 0[^}]*\}/)
    expect(text).not.toMatch(/\.dark\{[^}]*--fifth:2 34 34[^}]*\}/)
    expect(text).not.toMatch(/\.light, :root\{[^}]*--fifth:3 51 51[^}]*\}/)
